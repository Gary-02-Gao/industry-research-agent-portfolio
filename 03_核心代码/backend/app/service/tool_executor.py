...
from service.action_gate import ActionGateRequest, evaluate_action, permissions_from_context
...
class ToolExecutor:
    ...
    def __init__(
        ...
        self.llm_client = create_joy_agent_client(api_key=llm_api_key, base_url=llm_base_url)
    ...
    async def execute(self, tool_name: str, params: Dict[str, Any], context: ReActContext) -> Any:
        ...
        decision = evaluate_action(ActionGateRequest(
            user_intent=str(context.query or "unspecified user request"),
            permissions=permissions_from_context(context.metadata),
            tool_name=tool_name,
            params=dict(params),
        ))
        context.metadata.setdefault("action_gate", []).append(decision.model_dump(mode="json"))
        if not decision.allowed:
            raise PermissionError(f"Action Gate blocked {tool_name}: {decision.reason_code}")
    ...
    def _websearch_sync(self, query: str, count: int = 5) -> List[Dict]:
        ...
        try:
            ...
            # 过滤和格式化结果
            results = []
            for item in value_list[:count]:
                if item.get('link') and item.get('snippet'):
                    results.append({
                        'url': item.get('link'),
                        'name': item.get('title', 'N/A'),
                        'summary': item.get('snippet', ''),
                        'snippet': item.get('snippet', ''),
                        'siteName': item.get('source', 'N/A'),
                        'siteIcon': '',
                        'source': 'web'
                    })
